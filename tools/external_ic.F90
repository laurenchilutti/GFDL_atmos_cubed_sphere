    ...
    contains
      ...
      subroutine read_gfs_data_bad()
        ...
        if( open_file(GFS_restart, fn_gfs_ics, "read", Atm%domain, is_restart=.true., dont_add_res_to_filename=.true.) ) then
          ...
          ! GFS grid height at edges (including surface height)
          call register_restart_field(GFS_restart, 'zh', zh_tmp, (/"lev","lat", "lon"/))
          ! real temperature (K)
          call register_restart_field(GFS_restart, 't', temp_tmp, is_optional=.true.)

          ! Prognostic tracers
          do nt = 1, ntracers
            call get_tracer_names(MODEL_ATMOS, nt, tracer_name)
            call register_restart_field(GFS_restart, trim(tracer_name), q_tmp(:,:,:,nt), is_optional=.true.)
          enddo
...
!--- read in ak and bk from the HRRR control file using fms_io read_data ---
      ...
      allocate ( v_s(is:ie, js:je+1, 1:levp) )
        ...
        if (Atm%neststruct%nested) then
           ...
           fn_hrr_ics = fn_hrr_ics(1:len_trim(fn_hrr_ics)-2)//trim(suffix)//".nc"
        endif
...
!--- read in surface temperature (k) and land-frac
       ...
       if( open_file(SFC_restart, fn_sfc_ics, "read", Atm%domain, is_restart=.true., dont_add_res_to_filename=.true.) ) then
        ...
        if( open_file(HRRR_restart, fn_hrr_ics, "read", Atm%domain,is_restart=.true., dont_add_res_to_filename=.true.) ) then
          ...
          call register_axis(HRRR_restart, 'lev', 64)
          call register_axis(HRRR_restart, 'levp', 65)
          call register_axis(HRRR_restart, 'ntracer', 3)
          ...
          do nt = 1, ntracers
            call get_tracer_names(MODEL_ATMOS, nt, tracer_name)
            call register_restart_field(HRRR_restart, trim(tracer_name), q(:,:,:,nt), is_optional=.true.)
          enddo
