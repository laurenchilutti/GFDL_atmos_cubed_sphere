...
! Share the load
          ...
          if(mpp_pe()==1) call pmaxmin( 'U_data',   u0(:,:,1), im*jm, km, 1.)
          if(mpp_pe()==1) call pmaxmin( 'V_data',   v0(:,:,1), im*jm, km, 1.)
          if(mpp_pe()==2) call pmaxmin( 'T_data',   t0(:,:,1), im*jm, km, 1.)
          if(mpp_pe()==3) call pmaxmin( 'DEL-P',   dp0(:,:,1), im*jm, km, 0.01)
...
! D to A transform on lat-lon grid:
      ...
      if(mpp_pe()==4) call pmaxmin( 'UA', ua(:,:,1), im*jm, km, 1.)
      if(mpp_pe()==4) call pmaxmin( 'VA', va(:,:,1), im*jm, km, 1.)
