...
use fms2_io_mod,   only: open_file, close_file, get_num_dimensions, &
                         ...
                         get_time_calendar, read_data
use mpp_mod,       only: input_nml_file
use mpp_io_mod,    only: mpp_get_default_calendar
...
type filedata_type
  ...
  character(len=128), dimension(NUM_REQ_FLDS+1) :: axes
  character(len=128), dimension(NUM_REQ_FLDS) :: fields
end type
...
!   ntime       number of time levels
  ...
  character(len=128), allocatable :: fields(:)
...
character(len=128), save:: time_axis
